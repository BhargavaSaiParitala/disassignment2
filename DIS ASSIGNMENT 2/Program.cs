            ...
            try
            {
                ...
                foreach (int num in nums)
                {
                    if (num > num_ranges)
                    {
                        if (num - num_ranges == 1)
                        {
                            lstresult.Add(new List<int> { (int)num_ranges, (int)num_ranges});
                        }
                        else if (num - num_ranges > 1)
                        {
                            lstresult.Add(new List<int> { (int)num_ranges, (int)(num - 1) });
                        }
                    }

                    num_ranges = (long)num + 1;
                }

                if (num_ranges <= upper)
                {
                    if (upper - num_ranges == 0)
                    {
                        lstresult.Add(new List<int> { (int)num_ranges, (int)num_ranges });
                    }
                    else if (upper - num_ranges > 0)
                    {
                        lstresult.Add(new List<int> { (int)num_ranges, (int)upper });
                    }
                }
        ...
        */
        ...
        public static bool IsValid(string s)
        {
            try
            {
                ...
                var CharOrder = s.ToCharArray();

                for (int i = 0; i < CharOrder.Length; i++)
                {
                    if (CharOrder[i] == '(' || CharOrder[i] == '[' || CharOrder[i] == '{')
                    {
                        stk.Push(CharOrder[i]);
                        continue;
                    }
                    else if (stk.Count == 0)
                    {
        ...
        public static bool IsStrobogrammatic(string s)
        {
            try
            {
                ...
                while (left <= right)
                {
                    ...
                    if (!Pairs.ContainsKey(leftCharacter) || Pairs[leftCharacter] != rightCharacter)
                    {
                        return false;
                    }
                    left++;
        ...
        public static int NumIdenticalPairs(int[] nums)
        {
            try
            {
                ...
                foreach (int num in nums)
                {
                    ...
                    {
                        identicalPairs += countDictionary[num];
                        countDictionary[num]++;
                    }
                    else
                    {
        ...
        public static IList<string> GeneratePossibleNextMoves(string currentState)
        {
            try
            {
                ...
                for (int i = 0; i < currentState.Length - 1; i++)
                {
                    ...
                    {
                        string State = currentState.Substring(0, i) + "--" + currentState.Substring(i + 2);
                        Move.Add(State);
                    }
                }
